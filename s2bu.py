...
import math
import os
os.environ['PYGAME_HIDE_SUPPORT_PROMPT'] = 'hide' # Suppress "Hello from pygame" text in console
import pygame
pg = pygame
import pickle
...
class Game():
  def __init__(self):
    ...
    self.holdable = set([pg.K_LCTRL, pg.K_h, pg.K_g, pg.K_UP, pg.K_DOWN, pg.K_LEFT, pg.K_RIGHT])
  ...
  def draw_unit(self, k, l):
    ...
    if self.selection1 and self.selection2:
      ...
      if k1 <= k <= k2 and l1 <= l <= l2: #har funnits sedan python1
        r, g, b = color
        r //= 2
        g //= 2
        b //= 2
        color = (r, g, b)
    pygame.draw.rect(screen, color, pygame.Rect(sx,sy,sxp-sx,syp-sy))
    pygame.draw.line(screen, (0,0,0), (sx,sy), (sx,syp))
    pygame.draw.line(screen, (0,0,0), (sx,sy), (sxp,sy))

  def draw_all(self):
    for k in range(int(math.floor(self.x1)), int(math.ceil(self.x2))+1):
      for l in range(int(math.floor(self.y1)), int(math.ceil(self.y2))+1):
        # This is a performance bottleneck; consider incrementing in pixel space instead.
        self.draw_unit(k,l)
